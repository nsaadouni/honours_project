...
The mitigation for the PIN authentication protocol is still a difficult challenge, even with the above suggestions implemented. A users PIN must be still securely transferred between the API (middleware) and the smartcard for verification. Using the threat of a physical man in the middle attack being present, a protocol that is not vulnerable to a man in the middle and off-line attack is required. A protocol we mange to find that may help mitigate this issue (using software, not hardware), is the use of password authenticated key agreement (PAKE) \cite{hao2008password} protocol. It is one of the few methods that states to offer this level of protection.
