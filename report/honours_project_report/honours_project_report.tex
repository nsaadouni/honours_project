...
%Thus we now know that the public key of the card is actually one bytes of value 5, followed by 2 lots of 128 bytes worth of data.
...
There are three main protocols that would support session keys being derived that would remain a secret between the API and the card. These are: RSA, elliptic curve cofactor diffie hellman (ECCDH) and diffie hellman (DH) [see section 3]. Within security policies for 'Athena smartcard solutions' it states that ECCDH is used for cards from 2012 onwards. The card we analyse supports the key generation of elliptic curve public and private keys for this exact purpose, and for signing and verifying data. Thus the first protocol we look into is ECCDH.
