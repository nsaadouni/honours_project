...
From this analysis of the communication trace it seems that the card is generating the initial bytes form a 'get challenge' request (within secure messaging) to generate the bytes which will be the key value. As both the generation of those bytes and the creation of the key file is completed using secure messaging, the value is not sent over in plain text. 
