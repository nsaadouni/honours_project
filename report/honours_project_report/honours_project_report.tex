...
\hline
Wrap/Decrypt attack at the APDU layer & - & \checked\\
\hline
Modifying the retry counter & C\_login & \checked\\
\hline
\end{tabular}
\end{table}
...
The experiment consists of two tests. Both of the tests implement the above man in the middle attack setting on the diffie hellman protocol to set the shared secret value to 0. Test 1 sets the first 16 bytes of the 32 bytes the card sends to 0. Test 2 sets the second 16 bytes to 0.
...
\begin{enumerate}
\item Obtain the knowledge of the encrpyted and unencrypted command4
\item Obtain the knowledge of how to build the encrypted command
\item Build the encrypted command with keys we generate from a certain key derivation function \textbf{KDF} (using both the shared secret and the 32 bytes from the smartcard)
\item Test keys with different block ciphers
