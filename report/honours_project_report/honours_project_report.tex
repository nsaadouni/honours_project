...
%This is analogous to C code being compiled down to binary data to be operated on by the CPU. The addition of two integers cannot be considered correct in C, unless the corresponding binary instructions sent to the CPU are correct as well.
...
\item The status of a user being authenticated (correct PIN) should be controlled by smartcard's hardware. And prevent invalid function requests if their authentication status is false.
\end{itemize}
