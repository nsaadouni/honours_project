...
\section{Reverse Engineering PIN Authentication Protocol} \label{sect:boom}
...
Mitigations suggested in literature \cite{apdu:attacks} are more reasonable for smartcard manufacturers to implement at a lower cost. These include:
\begin{enumerate}
...
\item Using the OTP functionality of some hardware security modules (usb tokens) for PIN authentication protocols. This should make it far more difficult for an attacker to reverse engineer the PIN authentication protocol. In the case they do, the attacker will still require physical access to the device. \textit{This would however only apply to that of hardware secuirty modules with the OTP functionality implemented and have a display unit.}

\item Running the middleware on a different process and at a higher privilege than the user of the system. This should prevent an attacker from attaching to the process and observing/ altering APDU communication.
\end{enumerate}

(move) does not prevent an attacker from gaining the understanding of the smartcards middleware implementation on his own computer (as he can give himself the same privilege level as the middleware). With that understanding an attacker could then send APDU commands directly to the smartcard on a users infected machine without the use of the middleware. Thus rendering the approach in-applicable to those forms of attacks.
