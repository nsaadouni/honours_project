...
Furthermore detection and prevention of man in the middle attacks should be implemented. This will seriously halt the ability to gain the understanding required for the APDU level attacks discussed, if implemented correctly. However there is no guarantee that detection and prevention is possible for man in the middle attacks that are automated by software. Considering clock synchronization between two devices always incurs a lag, it is possible that a software man in the middle attack could go undetected.
