...
Just like the destroyObject function we omit the APDU communication for login authentication and finding the object to use for encryption. These are still essential parts to the correct use of the encrypt function though. 
...
\item Create key value file (B.10 - command 126)
...
The CLA and and INS bytes are identical for all of the commands. From this we inferred that if the wrap APDU command were to still exist and it is only the API that has been modified to refuse to send the wrap command and instead throw an error. That the command for wrapping at the APDU level would be: \textbf{00 2a 82 0a L Data 00}. Were L is the length of the data, and the data is the key to be wrapped. 
