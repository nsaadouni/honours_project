...
The second dependancy on the use of the API, is secure messaging. Every time a key is generated, whether it be on the card, or on the computer and transfered, secure messaging is utilised to input the key into a file on the card. Previous hardware security modules from the manufacture 'Athena- Smartcard solutions' (specifically ASE KeyPro) had vulnerabilities at the APDU layer that needlessly exported in plain text encryption keys [?]. As discussed in the analysis of this cards API functions in chapter 6, that vulnerability appears to be patched in this version of their smartcard, by using secure messaging. If this protocol can be reversed engineered, it should be possible to open files containing the keys and decrypt them. And also decrypt keys as they are being generated.
