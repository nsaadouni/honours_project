...
"""
try doing password = md5(key1_des3)
decrypt(Y, password, ECB)
"""
...
def test_2():
	...
	Y = ba.unhexlify(''.join(tmp))
	##############################################################

	# DES-MAC
	print ''.join(tmp)[0:16] + ' ' + ''.join(tmp)[16:32]

	cipher2 = DES3.new(key1_3des, DES3.MODE_ECB, IV=''.join(Y)[0:8])

	ciphertext1 = cipher2.decrypt(Y)
	p1 = ciphertext1[0:8]
	p2 = ciphertext1[8:16]
